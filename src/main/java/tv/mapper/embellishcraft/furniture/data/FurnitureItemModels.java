...
{
    public FurnitureItemModels(PackOutput output, String modid, ExistingFileHelper existingFileHelper, String name)
    {
    ...
    protected void registerModels()
    {
        ...
        for(int j = 0; j < Arrays.stream(McWoods.values()).count(); j++)
        {
            ...
            getBuilder(McWoods.byId(j).getSerializedName() + "_sturdy_table").parent(new UncheckedModelFile(modid + ":block/" + McWoods.byId(j).getSerializedName() + "_sturdy_table_inventory"));
        }
        ...
        for(int j = 0; j < Arrays.stream(McWoods.values()).count(); j++)
        {
            ...
            // {
            // getBuilder(DyeColor.byId(i).getSerializedName() + "_" + McWoods.byId(j).getSerializedName() + "_fancy_bed").parent(new UncheckedModelFile(modid + ":block/" +
            // DyeColor.byId(i).getSerializedName() + "_" + McWoods.byId(j).getSerializedName() + "_fancy_bed_inventory"));
            // }
        }
