        ...
        switch(lang)
        {
            ...
            case "en_us":
                for(int j = 0; j < Arrays.stream(McWoods.values()).count(); j++)
                {
                    ...
                    provider.addBlock(InitFurnitureBlocks.STURDY_TABLE_BLOCKS.get(McWoods.byId(j)), name + " Sturdy Table");
            ...
            case "fr_fr":
                ...
                for(int j = 0; j < Arrays.stream(McWoods.values()).count(); j++)
                {
                    ...
                    de = vowels.indexOf(name.charAt(0)) != -1 ? " d'" : " de ";
                    ...
                    provider.addBlock(InitFurnitureBlocks.STURDY_TABLE_BLOCKS.get(McWoods.byId(j)), "Table en bois massif" + de + name);
