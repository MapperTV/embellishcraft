...
public class InitFurnitureItems
{
    ...
    public static final Map<DyeColor, RegistryObject<BlockItem>> WARPED_FANCY_BED_ITEMS = Arrays.stream(DyeColor.values()).map(type -> Pair.of(type, FURNITURE_ITEM_REGISTRY.register(type.getSerializedName() + "_warped_fancy_bed", () -> new BlockItem(InitFurnitureBlocks.WARPED_FANCY_BED_BLOCKS.get(type).get(), new Item.Properties().tab(ModItemGroups.EMBELLISHCRAFT_FURNITURE))))).collect(Collectors.toMap(Pair::getKey, Pair::getValue));
    public static final Map<DyeColor, RegistryObject<BlockItem>> CRIMSON_FANCY_BED_ITEMS = Arrays.stream(DyeColor.values()).map(type -> Pair.of(type, FURNITURE_ITEM_REGISTRY.register(type.getSerializedName() + "_crimson_fancy_bed", () -> new BlockItem(InitFurnitureBlocks.CRIMSON_FANCY_BED_BLOCKS.get(type).get(), new Item.Properties().tab(ModItemGroups.EMBELLISHCRAFT_FURNITURE))))).collect(Collectors.toMap(Pair::getKey, Pair::getValue));
