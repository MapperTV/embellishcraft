    ...
    public static void addTranslations(ECLang provider, String lang)
    {
        ...
        switch(lang)
        {
            ...
            case "fr_fr":
                for(int j = 0; j < Arrays.stream(DyeColor.values()).count(); j++)
                {
                    provider.addBlock(InitLightBlocks.TABLE_LAMP_BLOCKS.get(DyeColor.byId(j)), "Lampe de chevet " + ECConstants.frColorsF[j]);
                    provider.addBlock(InitLightBlocks.MANUAL_TABLE_LAMP_BLOCKS.get(DyeColor.byId(j)), "Lampe de chevet " + ECConstants.frColorsF[j] + " (manuelle)");
                }
