...
public class ECBlockModels extends BaseBlockModels
{
    ...
    protected void registerModels()
    {
        ...
        for(int j = 0; j < Arrays.stream(McWoods.values()).count(); j++)
        {
            ...
            getBuilder(McWoods.byId(j).getString() + "_large_suspended_stairs").parent(getExistingFile(modLoc("block/large_suspended_stairs"))).texture("all", new ResourceLocation("block/" + McWoods.byId(j).getString() + "_planks"));
